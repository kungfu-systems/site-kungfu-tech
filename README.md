...
its Coming Soon label, slide status, and controls sit below the card so they do
not consume the card's first-screen width budget. Later demonstrations must be
added as independently source-bound slides rather than fake placeholders or
hand-pinned media. The Agent Work Lab video's poster, transcript, duration, and
public evidence path are generated by `scripts/import-auditable-demo.mjs` from
the retained Passport and closed media bundle. Only the active slide may play;
...
header, card chrome, and a small bottom margin; the secondary carousel controls
may follow below the fold on especially short browser viewports. This keeps the
complete card inside the first browser viewport even when a 4K monitor uses
