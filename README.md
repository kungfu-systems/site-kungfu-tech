...
The homepage places its demonstration reel on a full-width row beneath the
continuity-first message. The reel is a multi-slide carousel with previous/next,
keyboard, and touch-swipe navigation; a single published slide truthfully shows
`01 / 01` with disabled edge controls. Add later demonstrations as independently
source-bound slides rather than fake placeholders or hand-pinned media. The
current Agent Work Lab video's poster, transcript, duration, and public evidence
path are generated by `scripts/import-auditable-demo.mjs` from the retained
Passport and closed media bundle. Only the active slide may play; playback
starts through the reduced-motion-aware controller, remains muted and
user-controllable, and falls back to the exact poster when a browser blocks it.
