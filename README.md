...
retained Passport and closed media bundle. Independent qualified native frame
sets supply source-resolution 1080p MP4/WebM from a 150x36 PTY and exact
1280x720 MP4/WebM from a 100x28 PTY for constrained screens; the importer
rejects a 720p output derived by scaling the primary frame set. `<source media>`
selection is derived from the Passport's rendition roles, never guessed from
filenames. The 720p GIF remains the README compatibility rendition. Only the
active slide may play;
...
retains the selected media profile, qualification root, exact role-to-file
coordinates, native frame-set dimensions, and distinct capture roots so
responsive delivery cannot silently diverge from the qualified bundle.
