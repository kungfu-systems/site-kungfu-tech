...
media. The Agent Work Lab video's poster, transcript, duration, and public
evidence path are generated by `scripts/import-auditable-demo.mjs` from the
retained Passport and closed media bundle. The same qualified frame set supplies
source-resolution 1080p MP4/WebM for desktop and exact 1280x720 MP4/WebM for
constrained screens; `<source media>` selection is derived from the Passport's
rendition roles, never guessed from filenames. The 720p GIF remains the README
compatibility rendition. Only the active slide may play;
...
width and the dynamic viewport height. The height cap reserves only the shared
...
homepage's primary visual field look artificially narrow. In the light theme it
...
text left-aligned, and stacks the Agent-native loop and Agent Builders links as
...
The `0.1.0-alpha.14` npm tarball authenticates its current brand and evidence
...
and `@kungfu-tech/buildchain@3.0.3`. This repository owns only site layout,
