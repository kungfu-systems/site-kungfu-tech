...
The homepage pairs the continuity-first message with the exact qualified Agent
Work Lab replay. Its video, poster, transcript, duration, and public evidence
path are generated by `scripts/import-auditable-demo.mjs` from the retained
Passport and closed media bundle; do not hand-pin a second demo coordinate in
the homepage. Playback starts only through the reduced-motion-aware page
controller, remains muted and user-controllable, and falls back to the exact
poster when a browser blocks playback. The adjacent language must preserve the
bounded offline replay and non-authority boundaries rather than presenting the
demo as provider, durability, or production proof.
