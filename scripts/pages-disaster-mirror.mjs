...
const PRIMARY_SURFACES = new Map([["kungfu.tech", ""]]);
...
function canonicalFor(relative) {
  if (relative === "index.html") return "https://kungfu.tech/";
  if (relative.endsWith("/index.html")) return `https://kungfu.tech/${relative.slice(0, -10)}`;
  return `https://kungfu.tech/${relative}`;
}
...
function incidentPage(primaryHost, mirrorHost) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="index, follow"><link rel="canonical" href="https://${mirrorHost}/incident/"><title>Kungfu product site disaster recovery entry</title></head><body><aside ${BANNER_MARKER} role="status" style="padding:.7rem 1rem;background:#fff3cd;border-bottom:1px solid #d6b84a;text-align:center;font:600 14px/1.4 system-ui,sans-serif;">Disaster mirror — non-canonical and read-only.</aside><main style="max-width:52rem;margin:3rem auto;padding:0 1.25rem;font:16px/1.6 system-ui,sans-serif;"><h1>Kungfu product site disaster recovery</h1><p>This is the stable incident entry for <strong>${mirrorHost}</strong>. It is intentionally discoverable, but the mirrored product pages remain non-canonical copies of <a href="https://${primaryHost}/">${primaryHost}</a>.</p><ol><li>Confirm the primary site is unavailable from more than one network.</li><li>Check <a href="/.well-known/kungfu-mirror-status.json">mirror status</a> and its SHA-256 sidecar.</li><li>Use this mirror for read-only documentation and downloads only; the primary URL remains canonical.</li><li>Do not publish, qualify, or infer production health from mirror availability.</li><li>When the primary recovers, return readers to <a href="https://${primaryHost}/">https://${primaryHost}/</a>.</li></ol><p>This mirror is promoted only from a Buildchain production passport and the exact artifact from the same successful workflow run.</p></main></body></html>`;
}
...
async function main() {
  ...
  if (command === "verify") {
    const result = await verifyMirrorBase(required(values, "base-url"), {
      ...
      routes: (values.routes || "/,/why-kungfu/,/how-tested/continuity/,/how-tested/auditable-demo/,/agent-builders/,/agent-supply-chain/,/agent-hub/,/about/,/incident/").split(","),
    });
    process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
    return;
  }
