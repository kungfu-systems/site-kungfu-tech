...
grep -Fq 'Generated by site-kungfu-tech' public/install.sh
grep -Fq 'Generated by site-kungfu-tech' public/install.ps1
grep -Fq 'verify_bootstrap_candidate' public/install.sh
grep -Fq 'verify_bootstrap_candidate' public/install.ps1
grep -Fq 'alpha2-bootstrap-adapter-receipt.json' public/install.sh
grep -Fq 'alpha2-bootstrap-adapter-receipt.json' public/install.ps1
grep -Fq -- '--continue-at -' public/install.sh
grep -Fq 'RangeHeaderValue' public/install.ps1
node - <<'NODE'
const fs = require("node:fs");
const catalog = JSON.parse(fs.readFileSync("public/.well-known/kungfu/managed-installer.json", "utf8"));
if (
  catalog.schema !== "kungfu.site-managed-installer-catalog/v1" ||
  !/^sha256:[a-f0-9]{64}$/.test(catalog.catalogRoot || "") ||
  catalog.release?.version !== "4.0.0-alpha.2" ||
  catalog.transaction?.resume !== "http-range" ||
  catalog.transaction?.activation !== "atomic-current-previous" ||
  catalog.trust?.compatibilityAdapter?.mode !== "signed-alpha2-field-projection" ||
  catalog.implementation?.schema !== "kungfu.site-managed-installer-implementation/v1" ||
  !/^sha256:[a-f0-9]{64}$/.test(catalog.implementation?.generator || "") ||
  !/^sha256:[a-f0-9]{64}$/.test(catalog.implementation?.templates?.["install.sh"] || "") ||
  !/^sha256:[a-f0-9]{64}$/.test(catalog.implementation?.templates?.["install.ps1"] || "") ||
  catalog.entries?.map((entry) => `${entry.platform}/${entry.architecture}`).join(",") !==
    "darwin/arm64,linux/x64,win32/x64" ||
  catalog.entries.find((entry) => entry.platform === "linux")?.compatibility?.minimumGlibc !== "2.39"
) {
  throw new Error("site-managed installer catalog contract drifted");
}
for (const name of ["install.sh", "install.ps1"]) {
  const source = fs.readFileSync(`public/${name}`);
  const immutable = fs.readFileSync(
    `public/installers/site/v1/alpha/${catalog.release.version}/${catalog.catalogRoot.slice(7)}/${name}`,
  );
