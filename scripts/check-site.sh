...
node --test scripts/generate-managed-installers.test.mjs
...
grep -Fq '4.0.0-alpha.3' public/install.sh
grep -Fq '4.0.0-alpha.3' public/install.ps1
node - <<'NODE'
const fs = require("node:fs");
const publication = JSON.parse(fs.readFileSync("public/installer-publication.json", "utf8"));
if (
  publication.schema !== "kungfu.bootstrap-installer-publication/v1" ||
  publication.entries?.map((entry) => `${entry.platform}/${entry.architecture}`).sort().join(",") !==
    "darwin/arm64,linux/arm64,linux/x64,win32/x64"
) throw new Error("Alpha.3 installer publication contract drifted");
for (const asset of publication.assets || []) {
  ...
  if (
    ...
    !bytes.equals(fs.readFileSync(`public/${publication.immutablePath}/${asset.name}`))
  ) {
    throw new Error(`${asset.name} differs from Alpha.3 publication metadata`);
  }
}
NODE
